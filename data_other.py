    ...
    def update(self, timestamp, position):
        ...
        if self._timestamp.current == None or (
            ...
            if self._timestamp.previous != None:
                ...
                calculated_hero_data = self._hero_database.get_calculated_data(
                    timestamp
                )
                vector_hero_other = Operations.get_vector(
                    calculated_hero_data.position,
                    self._calculated_data[timestamp].position,
                )
                distance_to_hero = Operations.get_vector_length(vector_hero_other)
                angle_to_hero = Operations.get_angle_between_vectors(
                    calculated_hero_data.orientation,
                    self._calculated_data[timestamp].orientation,
                )

                calculated_data = CalculatedData(
                    position, orientation, velocity, distance_to_hero, angle_to_hero
                )
